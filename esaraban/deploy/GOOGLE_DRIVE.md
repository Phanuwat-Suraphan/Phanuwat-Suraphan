...
1. ไปที่ <https://myaccount.google.com/permissions> → หาแอปของคุณ → **เพิกถอนสิทธิ์ (Remove access)**
   — ขั้นนี้ห้ามข้าม ถ้าไม่เพิกถอน Google จะถือว่าเคยยินยอมแล้วและจะ**ไม่ส่ง refresh token ตัวใหม่กลับมา**
2. กลับมาที่เว็บ → เมนู **"เชื่อมต่อ Google Drive"** → กด **"เชื่อมต่อบัญชี Google"** → อนุญาตอีกครั้ง
3. คัดลอกค่า `GOOGLE_OAUTH_REFRESH_TOKEN` ตัวใหม่ที่ได้
4. เอาไปทับค่าเดิมใน Render: Dashboard → service → **Environment** → แก้ตัวแปร
   `GOOGLE_OAUTH_REFRESH_TOKEN` → **Save Changes** (Render จะ restart ให้เอง)
5. รอ deploy เสร็จ แล้วเปิดหน้า **"💾 สำเนาสำรองข้อมูล"** — ภายในไม่เกิน 5 นาทีต้องเห็นสำเนาของ
