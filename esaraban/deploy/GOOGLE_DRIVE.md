...
### แถบเตือนเมื่อยังไม่ได้เชื่อมต่อ

ตราบใดที่ยังไม่ได้เชื่อม Drive แดชบอร์ดของ**ผู้ดูแลและธุรการ**จะขึ้นแถบเตือน (ครูทั่วไปไม่เห็น
เพราะแก้ไม่ได้อยู่แล้ว) ความแรงของคำเตือนขึ้นกับชนิดดิสก์ของเครื่อง:

- **ดิสก์ถูกล้างทุกครั้งที่ deploy** (เช่น Render free tier) → แถบแดง **ปิดไม่ได้** บอกตรงๆ ว่า
  ข้อมูลจะหายในการ deploy ครั้งถัดไป
- **ดิสก์ถาวร** (เครื่องโรงเรียน / VPS) → แถบเหลืองที่กด "รับทราบแล้ว" เก็บได้

ระบบเดาชนิดดิสก์จากตัวแปรที่ Render ตั้งให้เอง ถ้าเดาไม่ตรงกับความจริง ให้ตั้ง `EPHEMERAL_DISK`
เป็น `1` (ดิสก์ถูกล้าง) หรือ `0` (ดิสก์ถาวร) — ค่าที่ตั้งเองชนะการเดาเสมอ
...
| ช่อง | กรอกอะไร |
|---|---|
| **App name** | ชื่อระบบพร้อมชื่อโรงเรียน เช่น `ระบบสารบรรณอิเล็กทรอนิกส์ โรงเรียนบ้านห้วยแก้ว` (ใช้ชื่อเดียวกับที่ตั้งไว้ในหน้า "ตั้งค่าโรงเรียน") |
