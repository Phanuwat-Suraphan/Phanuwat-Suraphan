...
// กล่องความเห็น/ลงนามของผู้อำนวยการ — อ้างอิงรูปแบบตรายางจริงของโรงเรียน (มุมขวาล่าง กรอบสี่เหลี่ยม:
// ชื่อตำแหน่ง + checkbox ทราบ/อนุญาต-ไม่อนุญาต/อนุมัติ-ไม่อนุมัติ + "เห็นควรให้..." + ลายเซ็น + ชื่อ-ตำแหน่ง)
// decision: 'acknowledge' | 'approve' | 'reject' — ตรงกับปุ่ม รับทราบ/อนุมัติและส่งต่อ/ไม่อนุมัติ ในหน้าเอกสาร
export async function stampDirectorDecision({ originalBuffer, schoolName, decision, note, signatureDataUrl, prefix, firstName, lastName, position, dateThaiLong, xPercent, yPercent }) {
  const mark = (on) => (on ? '●' : '○');
  const leftPt = Math.max(0, Math.min(70, xPercent ?? 55)) / 100 * PAGE_WIDTH_PT;
  const topPt = Math.max(0, Math.min(75, yPercent ?? 66)) / 100 * PAGE_HEIGHT_PT;
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>
    @page { size: ${PAGE_WIDTH_PT}pt ${PAGE_HEIGHT_PT}pt; margin: 0; }
    body { margin: 0; font-family: "Noto Sans Thai", sans-serif; -webkit-print-color-adjust: exact; }
